...
export default async function readFiles(paths, options) {
    ...
    const results = await Promise.all(paths.map(async path => {
        ...
        if(stats.isDirectory()) {
            return await batchOperation((await readdir(path, {
                withFileTypes: true,
                recursive,
            }))
                ...
                async file => {
                    ...
                    const result = await loader(joinPath(root ? filePath.replace(root, '') : filePath, file.name), file.name);
                    if(postFilter(result))
                        return result;
                }
            );
        } else if(stats.isFile()) {
            const result = await loader(root ? path.replace(root, '') : path, basename(path));
            if(postFilter(result))
                return result;
        }
    }))
