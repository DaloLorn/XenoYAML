...
export default async function exportToArtitas(options) {
    ...
    const xenoyamlRegex = new RegExp("(.*\\" + sep + "xenoyaml\\" + sep + "?)?", "i");

    let xenoyamlRoot = projectRoot.match(xenoyamlRegex)[0];
    if(!xenoyamlRoot) {
        console.warn("Could not find content pack XenoYAML folder! Paths in the resulting templates will be relative to the project root, instead.");
    ...
    const outputFolder = customOutputFolder || resolvePath(xenoyamlRoot, '..', 'template');
    ...
    await batchOperation(
        ...
        async (packedFile) => {
            ...
            await writeFile(path, JSON.stringify(packedFile.template).replaceAll(/([^\\]":\s*)"(-?Infinity)"/gi, "$1$2"));
            console.log(`Exported ${packedFile.path}.json`);
            exported = true;
        }
    )
    if(!exported) 
        console.error("No importable files were found. Please make sure the provided path contains valid XenoYAML files.");
}